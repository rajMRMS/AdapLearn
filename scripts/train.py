...
parser.add_argument(
    "--start_epoch",
    help="epoch offset when saving models (for resume numbering)",
    type=int,
    default=0,
)
...
def main(args):
    ...
    # prepare optimizer before potentially loading optimizer state from checkpoint
    optim = torch.optim.AdamW(
        model.parameters(), lr=args.learning_rate, weight_decay=args.l2
    )

    # If user requested CUDA but torch doesn't have CUDA support, fall back to CPU
    if str(args.device).startswith("cuda") and not torch.cuda.is_available():
        print(
            "Warning: CUDA requested but not available or Torch not compiled with CUDA. Switching to cpu."
        )
        args.device = "cpu"

    # checkpoint loading (supports both plain state_dict and full checkpoints)
    epoch_offset = args.start_epoch
    if args.from_file:
        # load checkpoint onto the target device when possible, otherwise load to cpu
        map_location = args.device if (str(args.device).startswith("cuda") and torch.cuda.is_available()) else "cpu"
        ckpt = torch.load(args.from_file, map_location=map_location)
        # full checkpoint format (dict with model_state_dict, optimizer_state_dict, epoch)
        if isinstance(ckpt, dict) and (
            "model_state_dict" in ckpt or "optimizer_state_dict" in ckpt or "epoch" in ckpt
        ):
            if "model_state_dict" in ckpt:
                model.load_state_dict(ckpt["model_state_dict"])
            else:
                # fallback if someone saved a dict containing state dict directly
                model.load_state_dict(ckpt)
            if "optimizer_state_dict" in ckpt:
                try:
                    optim.load_state_dict(ckpt["optimizer_state_dict"])
                except Exception:
                    # optimizer state may not match exactly; continue anyway
                    pass
            epoch_offset = max(epoch_offset, int(ckpt.get("epoch", 0)))
        else:
            # assume ckpt is a plain state_dict for the model
            model.load_state_dict(ckpt)

    # move model to the selected device after loading state dict
    ...
    for epoch in range(1, args.n_epochs + 1):
        real_epoch = epoch + epoch_offset
        print("start epoch", real_epoch)
        model.train()
        ...
        if args.output_dir:
            model_path = os.path.join(
                args.output_dir, f"model-{real_epoch:03d}-{r['auc']:.4f}.pt"
            )
            print("saving snapshot to:", model_path)
            # save full checkpoint containing epoch, model and optimizer states and metrics
            torch.save(
                {
                    "epoch": real_epoch,
        ...
        if args.early_stop > 0 and real_epoch - best_epoch > args.early_stop:
